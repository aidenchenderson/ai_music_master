...
// convert presented string and fret data into expected frequency data
...
SessionGrade PlayAlongGrader::grade_session(const Track& track, const std::vector<BeatFrameData>& recorded_beats) {
    ...
    // conversion between bar and beat to string and fret
    std::map<std::pair<int,int>, std::pair<int,int>> expected_notes;
    for (int bar_idx = 0; bar_idx < track.bars.size(); ++bar_idx) {
        for (const auto& note : track.bars[bar_idx].notes) {
            expected_notes[{bar_idx, note.beat}] = {note.string_index, note.fret};
        }
    }
    ...
    for (const auto& beat : recorded_beats) {
        ...
        double expectedTimeMs = (beat.bar_index * 4 + beat.beat_index) * beat_duration_ms;
